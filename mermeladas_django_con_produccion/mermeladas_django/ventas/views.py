...
class ProductDeleteView(DeleteView):
    ...
    # --- BUENA PRÁCTICA: Añadir protección aquí también ---
        ...
        if form.is_valid() and formset.is_valid():
            ...
            for f in formset.forms:
                ...
                if not producto or not cantidad:
                    messages.error(request,
                                   "Se detectó una fila de producto vacía. Por favor, complétela o márquela para eliminar.")
            ...
            for producto, cantidad_total in productos_a_descontar.items():
                if cantidad_total > producto.stock:
                    messages.error(request,
                                   f"Stock insuficiente para {producto.nombre}. Se solicitan {cantidad_total} y hay {producto.stock} disponible(s).")
                    return render(request, "ventas/form.html", {"form": form, "formset": formset, "precios": precios,
                                                                "stock_map": json.dumps(stock_map)})
            ...
            # -------- 4) DESCONTAR STOCK POR FEFO --------
            for producto, cantidad_total in productos_a_descontar.items():
                ok = _descontar_por_FEFO(producto, cantidad_total)
                if not ok:
                    messages.error(request,
                                   f"No fue posible descontar {cantidad_total} de {producto.nombre} (FEFO). Stock inconsistente.")
